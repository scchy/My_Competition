...
def resplit_trainer(total_dataset, model, config, test_ratio=0.2, save_epoch_freq=5, wandb_flag=False):
    device = config.device
    model.to(device)
    # 对于分类任务, 我们常用cross-entropy评估模型表现.
    criterion = nn.CrossEntropyLoss(ignore_index=-1)  # 交叉熵计算时，label范围为[0, n_classes-1]
    ...
    random_nums = np.ceil(1 / test_ratio)
    change_n_epochs = n_epochs // random_nums
    # 全集作为验证
    # valid_loader = DataLoader(total_dataset, batch_size=config.batch_size, shuffle=True, num_workers=10)
    sp = 0
    for epoch in range(n_epochs):
        reset_flag = (epoch == 0) or ((epoch + 1) % change_n_epochs == 0)
        if reset_flag:
            sp += 1
            train_dataset, val_dataset = random_split(total_dataset, [train_size, val_size])
            train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=10)
            valid_loader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=True, num_workers=10)
        ...
        for x, y in train_pbar:
            ...
            loss_record.append(l_)
            train_accs.append(acc.detach().item())
            train_pbar.set_description(f'Epoch [{epoch+1}/{n_epochs}]({sp}|{lr_now=:.5f})')
            train_pbar.set_postfix({'loss': f'{l_:.5f}', 'acc': f'{acc:.5f}'})
        ...
        loss_record = []
        val_accs = []
        val_pbar = tqdm(valid_loader, position=0, leave=False)
        for x, y in val_pbar:
            x, y = x.to(device), y.to(device)
            with torch.no_grad():
                pred = model(x)
