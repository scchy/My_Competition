...
# func: run:
#   luxai-s3  advMain.py  advMain.py --seed 2025 -o replay.html
# ==========================================================================================
import json
...
def agent_fn(observation, configurations):
    ...
    player_str = 'player_0' if player == 0 else 'player_1'
    remainingOverageTime = observation.remainingOverageTime
    
    path_ = os.path.dirname(__file__)
    # model_d_s1 = os.path.join(path_, "test_models", 'DQN_LuxAI_s1_dqn_vs_random_v3_0227_bst')
    model_d_s1 = os.path.join(path_, "test_models", 'DQN_LuxAI_s1_dqn_vs_random_v2_0227_new_bst')
    if player_str == 'player_1':
        model_d_s1 = os.path.join(path_, "test_models", 'DQN_LuxAI_s1_dqn_vs_random_v3_0227_new_bst')
    config = Namespace(
        seed=202502,
        num_episode=100,
        batch_size=128,
    ...
    if step == 0:
        agent_dict[player] = DQN(
            ...
            random_flag=player_str == 'player_1',
            min_samples=config.min_samples
        )
        if not agent_dict[player].random_flag:
            agent_dict[player].load_model(model_d_s1, player=player_str)
