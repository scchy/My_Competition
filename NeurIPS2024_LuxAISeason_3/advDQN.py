...
class QNet(nn.Module):
    def __init__(self, info_state_dim, action_dim):
        ...
        self.unit_cnn_feature = nn.Sequential(
            nn.Conv2d(4, 8, 6, 3),
            nn.ReLU(),
            nn.Conv2d(8, 16, 4, 2),
            nn.ReLU(),
        ...
        self.state_v = nn.Sequential(
            nn.Linear(128, action_dim),
            nn.ReLU()
        )
        
        self.info_state_dim = info_state_dim
    ...
    def forward(self, x):
        ...
        pic_o = self.cnn_feature(pic_f)
        state_o = self.state_nn(state_info)
        unit_pic_o = self.unit_cnn_feature(unit_pic_f)
        state_v = self.state_v(torch.concat([unit_pic_o, state_o], axis=1))
        o = torch.concat([pic_o, unit_pic_o, state_o], axis=1)
        return self.head(o) - state_v.mean(axis=1, keepdim=True)
...
class DQN:
    def __init__(
        ...
        self.target_q = QNet(state_dim, action_dim).to(self.device)
    ...
    def _get_action_mask(self, unit_pos):
        ...
        if x == 0:
            mask[4] = 0 # left
        if x == self.env_cfg["map_width"] - 1:
            mask[2] = 0 # right
        if y == 0:
            mask[1] = 0 # up
        if y == self.env_cfg["map_height"] - 1:
            mask[3] = 0 # down
        return mask
                ...
                # if len(visible_relic_node_ids) > 0:
                    ...
                    if manhattan_distance <= 4:
                        ...
                        if len(valid_actions) >= 1:
                            action_type = np.random.choice(valid_actions.flatten()) 
    ...
    def param_smooth_fusion(self, smooth_agent):
        smooth_tau = 0.3
        for q_param, add_param in zip(self.q.parameters(), smooth_agent.q.parameters()):
            q_param.data.copy_(
                q_param.data * (1 - smooth_tau) + add_param.data * smooth_tau
            )
        for tar_q_param, tar_add_param in zip(self.target_q.parameters(), smooth_agent.target_q.parameters()):
            tar_q_param.data.copy_(
                tar_q_param.data * (1 - smooth_tau) + tar_add_param.data * smooth_tau
            )
